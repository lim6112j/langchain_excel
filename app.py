...
def load_excel_data(excel_path: str) -> Tuple[pd.DataFrame, pd.DataFrame]:
    try:
        ...
        # 시트 이름 확인 및 대소문자 무시
        policy_sheet = next((s for s in sheet_names if s.lower() == "정책"), None)
        ...
        missing_policy_columns = [col for col in required_policy_columns if col not in policy_df.columns]
        missing_sales_columns = [col for col in required_sales_columns if col not in sales_df.columns]
        
        if missing_policy_columns:
            raise ValueError(f"정책 시트에 필수 열이 없습니다: {', '.join(missing_policy_columns)}")
        if missing_sales_columns:
            raise ValueError(f"매출 시트에 필수 열이 없습니다: {', '.join(missing_sales_columns)}")
...
def apply_policy(state: AgentState) -> AgentState:
    ...
    for company in state["sales_data"]["업체명"].unique():
        company_sales = state["sales_data"][state["sales_data"]["업체명"] == company]
        company_policy = state["policy_data"][state["policy_data"]["업체명"] == company]
        ...
        for _, policy_row in company_policy.iterrows():
            ...
            for col in policy_row.index:
                if pd.notna(policy_row[col]) and isinstance(policy_row[col], str) and "확인필요" in policy_row[col]:
                    unclear_items.append(f"업체 '{company}'의 '{col}' 항목에 '확인필요' 표시가 있습니다: {policy_row[col]}")
    ...
    # 메시지 추가
    if unclear_items:
        state["messages"].append(AIMessage(content=f"다음 항목들이 불명확합니다:\n" + "\n".join(unclear_items)))
    else:
...
def generate_questions(state: AgentState) -> AgentState:
    ...
    state["messages"].append(AIMessage(content="다음 질문들을 통해 불명확한 항목을 명확히 할 수 있습니다:\n" + "\n".join(questions)))
...
def process_answer(state: AgentState, answer: str) -> AgentState:
    ...
    try:
        ...
        state["messages"].append(AIMessage(content=f"답변 분석 중 오류가 발생했습니다: {str(e)}"))
...
def router(state: AgentState) -> str:
    # 디버깅 정보 출력
    logger.info(f"라우터 호출: unclear_items={len(state.get('unclear_items', []))}, questions={len(state.get('questions', []))}, answers={len(state.get('answers', []))}, current_node={state.get('_current_node')}")
...
def create_graph() -> StateGraph:
    ...
    # 엣지 추가
    workflow.set_entry_point("load_and_apply_policy")
    ...
    workflow.add_edge("generate_questions", "wait_for_answer")
    workflow.add_conditional_edges(
        "wait_for_answer",
        router,
        {
...
def create_interface():
    ...
    with gr.Blocks(title="엑셀 정산 계산기") as demo:
        ...
        with gr.Row():
            excel_file = gr.File(label="엑셀 파일 업로드", file_types=[".xlsx", ".xls"])
        ...
        def process_file(file):
            ...
            try:
                ...
                if not chat_messages:
                    chat_messages = [{"role": "assistant", "content": "파일이 처리되었지만 메시지가 생성되지 않았습니다."}]
        ...
        def answer_question(message, state, chatbot):
            ...
            try:
                ...
                try:
                    ...
                    logger.error(f"그래프 실행 오류: {str(graph_error)}")
                    state["messages"].append(AIMessage(content=f"처리 중 오류가 발생했습니다: {str(graph_error)}"))
            except Exception as e:
                logger.error(f"답변 처리 중 오류: {str(e)}")
                state["messages"].append(AIMessage(content=f"답변 처리 중 오류가 발생했습니다: {str(e)}"))
        ...
        msg.submit(answer_question, [msg, state, chatbot], [chatbot, state]).then(lambda: "", None, msg)
